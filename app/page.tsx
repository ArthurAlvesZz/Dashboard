...
export default function SantaBronxOps() {
  ...
  return (
    <div className="flex h-[100dvh] bg-background text-foreground overflow-hidden font-sans">
      ...
      <aside className="hidden lg:flex w-64 border-r border-border bg-card flex-col shrink-0 z-20 transition-all duration-300">
        <div className="h-16 flex items-center px-6 border-b border-border w-full">
           <div className="flex items-center gap-3">
             <div className="w-8 h-8 bg-primary/10 border border-primary/20 text-primary rounded-lg flex items-center justify-center shadow-sm">
          ...
          {MENUS.map((menu) => {
            ...
            return (
              ...
              >
                ...
                <Icon className={cn("w-[18px] h-[18px] transition-colors group-hover:scale-110", isActive ? "text-primary scale-110" : "text-muted-foreground")} />
        ...
        <div className="p-4 w-full border-t border-border bg-card mt-auto flex justify-center">
          <div className="flex w-full items-center gap-3 px-2 py-2 rounded-lg hover:bg-accent transition-colors cursor-pointer">
        ...
        <header className="h-16 border-b border-border flex items-center justify-between px-4 sm:px-6 shrink-0 bg-background/80 backdrop-blur-xl relative z-10 w-full overflow-x-auto gap-4 custom-scrollbar">
          <div className="flex items-center gap-3 w-full max-w-md">
            ...
            <div className="relative w-full sm:w-72 hidden sm:block">
              <Search className="absolute left-3 top-1/2 w-4 h-4 -translate-y-1/2 text-muted-foreground/60" />
              <input 
                 type="text" 
                 placeholder="Search anything..." 
                 className="w-full h-9 bg-muted/40 border border-input rounded-lg pl-9 pr-4 text-sm text-foreground focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/20 transition-all placeholder:text-muted-foreground/50"
              />
              <div className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none">
                 <kbd className="hidden sm:inline-flex items-center gap-1 rounded bg-muted border border-border px-1.5 font-mono text-[10px] font-medium text-muted-foreground">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
             <button className="hidden sm:flex items-center gap-1.5 h-8 bg-primary hover:bg-primary/90 text-primary-foreground text-xs font-medium rounded-md px-3 transition-colors shadow-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
            ...
            <div className="relative group">
              ...
              {/* ... store dropdown content same as before ... */}
              <div className="absolute right-0 top-full mt-1 w-40 bg-card border border-border rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 overflow-hidden">
                 {stores.map(store => (
                   <div 
                     ...
                     className={cn(
                       "px-4 py-2 text-sm cursor-pointer hover:bg-accent text-foreground transition-colors",
                       selectedStore === store.id ? "bg-accent font-medium" : ""
                     )}
                   >
                     {store.name}
                   </div>
               ...
               <div className="absolute right-0 top-full mt-1 w-40 bg-card border border-border rounded-lg shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 overflow-hidden">
                 {dateFilters.map(df => (
                   <div 
                     ...
                     className={cn(
                       "px-4 py-2 text-sm cursor-pointer hover:bg-accent text-foreground transition-colors",
                       selectedDate === df.id ? "bg-accent font-medium" : ""
                     )}
                   >
                     {df.name}
                   </div>
